...
bool CFmfMap::Open(const char *szFilePath)
{
	...
	// ヘッダ情報を読む
	unsigned long int dwReadBytes;
	if(fread(&m_fmfHeader, sizeof(FMFHEADER), 1, fp) != 1)
	...
	// メモリ確保
	m_pLayerAddr = new char[m_fmfHeader.dwSize];
	if (m_pLayerAddr == NULL)
		goto error_return;

	// レイヤーデータを読む
	if(fread(m_pLayerAddr, m_fmfHeader.dwSize, 1, fp) != 1)
...
//-----------------------------------------------------------------------------
void* CFmfMap::GetLayerAddr(char byLayerIndex) const
{
	// メモリチェック、範囲チェック
	if ((m_pLayerAddr == NULL) || (byLayerIndex >= m_fmfHeader.byLayerCount))
		return NULL;

	char bySize = m_fmfHeader.byBitCount / 8;
...
//	エラー		= -1
//-----------------------------------------------------------------------------
int CFmfMap::GetValue(char byLayerIndex, unsigned long int dwX, unsigned long int dwY) const
{
	int nIndex = -1;
	...
	{
		// 8bit layer
		char* pLayer = (char*)GetLayerAddr(byLayerIndex);
		nIndex = *(pLayer + dwY * m_fmfHeader.dwWidth + dwX);
	}
	else
	{
		// 16bit layer	
		unsigned int* pLayer = (unsigned int*)GetLayerAddr(byLayerIndex);
		nIndex = *(pLayer + dwY * m_fmfHeader.dwWidth + dwX);
	}
...
//-----------------------------------------------------------------------------
void CFmfMap::SetValue(char byLayerIndex, unsigned long int dwX, unsigned long int dwY, int nValue)
{
	// 範囲チェック
	if (byLayerIndex >= m_fmfHeader.byLayerCount ||
		dwX >= m_fmfHeader.dwWidth ||
	...
	if (m_fmfHeader.byBitCount == 8)
	{
		// 8bit layer
		char* pLayer = (char*)GetLayerAddr(byLayerIndex);
		*(pLayer + dwY * m_fmfHeader.dwWidth + dwX) = (char)nValue;
	}
	else
	{
		// 16bit layer	
		unsigned int* pLayer = (unsigned int*)GetLayerAddr(byLayerIndex);
		*(pLayer + dwY * m_fmfHeader.dwWidth + dwX) = (unsigned int)nValue;
	}
}
...
// マップの横幅を得る
//-----------------------------------------------------------------------------
unsigned long int CFmfMap::GetMapWidth(void) const
{
...
// マップの高さを得る
//-----------------------------------------------------------------------------
unsigned long int CFmfMap::GetMapHeight(void) const
{
...
// チップの横幅を得る
//-----------------------------------------------------------------------------
char CFmfMap::GetChipWidth(void) const
{
...
// チップの高さを得る
//-----------------------------------------------------------------------------
char CFmfMap::GetChipHeight(void) const
{
...
// レイヤー数を得る
//-----------------------------------------------------------------------------
char CFmfMap::GetLayerCount(void) const
{
...
// レイヤーデータのビットカウントを得る
//-----------------------------------------------------------------------------
char CFmfMap::GetLayerBitCount(void) const
{
