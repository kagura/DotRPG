...
// ＤＸライブラリのバージョン
#define DXLIB_VERSION 0x306e
#define DXLIB_VERSION_STR _T( "3.06e" )
...
extern	int			MakeKeyInput( int MaxStrLength, int CancelValidFlag, int SingleCharOnlyFlag, int NumCharOnlyFlag, int DoubleCharOnlyFlag = FALSE ) ;			// 新しいキー入力データの作成
...
// Dx3D.cpp 又は DxGraphics2 関数プロトタイプ宣言

// 特殊用途関係
extern	D_IDirect3DDevice7* GetUseD3DDevObj( void ) ;																// 使用中のＤｉｒｅｃｔ３ＤＤｅｖｉｃｅ７オブジェクトを得る( Ver 3.00 以降は無効 )
extern	D_IDirect3DDevice9* GetUseDirect3DDevice9( void ) ;															// 使用中のＤｉｒｅｃｔ３ＤＤｅｖｉｃｅ９オブジェクトを得る
extern	int			RenderVertex( void ) ;																			// 頂点バッファに溜まった頂点データを吐き出す
...
extern	int			SetUseGraphZBuffer( int GrHandle, int UseFlag ) ;												// グラフィック専用のＺバッファを持つかどうかを設定する( 描画可能画像のみ可能 )
...
extern	int			ChangeMovieVolumeToGraph( int Volume, int GraphHandle ) ;									// ムービーのボリュームをセットする(0～255)
...
// 基本イメージデータのロード＋ＤＩＢ関係
extern	int			CreateGraphImageOrDIBGraph( const TCHAR *FileName, void *DataImage, int DataImageSize, int DataImageType /* LOADIMAGE_TYPE_FILE 等 */ , int BmpFlag, int ReverseFlag, BASEIMAGE *Image, BITMAPINFO **BmpInfo, void **GraphData ) ;												// 登録されている各種グラフィックローダ関数から、ＢＭＰデータもしくは GraphImageデータを構築する
...
extern	int			UpdateLayerdWindowForPremultipliedAlphaSoftImage( int SIHandle ) ;															// 乗算済みアルファのソフトウエアで扱うイメージを使用して UpdateLayerdWindow を行う( イメージはウインドウのクライアント領域と同じサイズである必要があります )
...
extern	int			UpdateLayerdWindowForPremultipliedAlphaBaseImage( BASEIMAGE *BaseImage ) ;																							// 乗算済みアルファの基本イメージを使用して UpdateLayerdWindow を行う
